...
TRAINING_SIZE = len(x_train_1)
# Maximum length of input is 'int + int' (e.g., '345+678'). Maximum length of
MAXLEN = dm.getMaxLen(x_train_1)
REVERSE = False
...
chars = dm.getDic(x_train_1) + list(" ")
ctable = CharacterTable(chars)
print(ctable.encode("HELLO", MAXLEN))

print('Total training sample :', len(x_train_1))

print('Vectorization...')

x1 = np.zeros((len(x_train_1), MAXLEN, len(chars)), dtype=np.bool)
x2 = np.zeros((len(x_train_2), MAXLEN, len(chars)), dtype=np.bool)
y = np.zeros((len(questions), MAXLEN, len(chars)), dtype=np.bool)

for i, sentence in enumerate(x_train_1):
    x1[i] = ctable.encode(sentence, MAXLEN)
for i, sentence in enumerate(x_train_2):
    x2[i] = ctable.encode(sentence, MAXLEN)
...
split_at = len(x1) - len(x1) // 10
(x_train_1, x1_val) = x1[:split_at], x1[split_at:]
(x_train_2, x2_val) = x2[:split_at], x2[split_at:]
...
branch1 = Input(shape = x_train_1.shape, name='main_input_1')
#x1 = Embedding(output_dim=512, input_dim=27, input_length=MAXLEN)(branch1)
#lstm_out1 = LSTM(32)(branch1)

branch2 = Input(shape = x_train_2.shape, name='main_input_2')
x = layers.concatenate([branch1, branch2])
print(x)
x = Reshape((20241, 540,))(x)
x = RNN(HIDDEN_SIZE, input_shape=x.shape)(x)

x = layers.RepeatVector(MAXLEN)(x)
for _ in range(LAYERS):
    x = RNN(HIDDEN_SIZE, return_sequences=True, kernel_regularizer=regularizers.l2(0.001))(x)

x = layers.TimeDistributed(layers.Dense(len(chars)))(x)
main_output = layers.Activation('softmax')(x)
model = Model(inputs=[branch1, branch2], outputs=[main_output])
...
for iteration in range(1, 2000):
    ...
    # errors.
    for i in range(10):
        ind = np.random.randint(0, len(x1_val))
