...
words, encoded_words = dm.getDataset()
...
def getModel(x_train, y_train, loss='mean_absolute_error', optimizer='adam'):
	...
	x_max_len = dm.getMaxLen(x_train)
	...
	filter_length = 2
	nb_filter = 4
	nb_input_series = len(x_train[0])
	model.add(Conv1D(2,2,activation='relu',input_shape=(len(x_train), len(x_train[0]))))
...
esCallBack = callbacks.EarlyStopping(monitor='loss', min_delta=0, patience=10, verbose=1, mode='auto')
